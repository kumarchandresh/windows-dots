...
function Test-PendingReboot {
  if (Get-ChildItem "HKLM:\Software\Microsoft\Windows\CurrentVersion\Component Based Servicing\RebootPending" -ErrorAction Ignore) {
    Write-Host "Reboot pending due to: Component Based Servicing" -ForegroundColor Yellow
    return $true
  }
  if (Get-Item "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired" -ErrorAction Ignore) {
    Write-Host "Reboot pending due to: WindowsUpdate\Auto Update" -ForegroundColor Yellow
    return $true
  }
  $pfr = Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager" -Name PendingFileRenameOperations -ErrorAction Ignore
  if ($pfr) {
    Write-Host "Reboot pending due to: PendingFileRenameOperations. Operations:" -ForegroundColor Yellow
    $pfr.PendingFileRenameOperations
    return $true
  }
  try { 
    ...
    if (($null -ne $status) -and $status.RebootPending) {
      Write-Host "Reboot pending due to: CCM Client" -ForegroundColor Yellow
      return $true
    }
  }
  catch {}
